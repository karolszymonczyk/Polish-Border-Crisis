"""
This module provides a twitter scraper for getting and saving tweets infromation in database
"""
import json
import time
import stweet as st
...
from utils import get_database, get_local_database, get_language, get_proxy, chunks
...
class TweetsRawOutput(MongoDBRawOutput):
    def export_raw_data(self, raw_data_list: List[RawData]):
        ...
        for raw in raw_data_list:
            ...
            data["download_datetime"] = pd.to_datetime(raw.download_datetime.format())
            data["created_at"] = pd.to_datetime(data["created_at"])
            requests.append(
                UpdateOne({"id_str": data["id_str"]}, {"$set": data}, upsert=True)
            )
...
def scrape_tweets(
    ...
    until: Arrow,
    language: Language,
    any_word: str or None,
    all_words: str or None,
    db: Database,
    usernames: List[str] or None = None,
    single_period: int = 60 * 60 * 24,
):
...
if __name__ == "__main__":
    ...
    for chunk in word_chunks:
        any_word = " ".join(chunk)
        phrase = all_words if len(any_word) == 0 else any_word
        print(f'Scraping tweets for "{phrase}"  (only original)...')
        downloaded_count = scrape_tweets(
            **config, all_words=all_words, any_word=any_word, db=get_local_database()
        )
        print(f'Downloaded {downloaded_count} tweets for "{phrase}"!')
