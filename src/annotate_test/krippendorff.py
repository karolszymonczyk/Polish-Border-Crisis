...
def _nominal_metric(
    v1: np.ndarray, v2: np.ndarray, dtype: Any = np.float64, **kwargs
) -> np.ndarray:  # noqa
...
def _ordinal_metric(
    ...
    **kwargs
    ...
    sums_between_indices = np.add.reduceat(np.append(n_v, 0), ranges.reshape(-1))[
        ::2
    ].reshape(*i1.shape)
...
def _coincidences(value_counts: np.ndarray, dtype: Any = np.float64) -> np.ndarray:
    ...
    unnormalized_coincidences = (
        value_counts[..., np.newaxis] * value_counts[:, np.newaxis, :] - diagonals
    )
    return np.divide(
        unnormalized_coincidences, (pairable - 1).reshape((-1, 1, 1)), dtype=dtype
    ).sum(axis=0)
...
def _distances(
    ...
    distance_metric: Callable[..., np.ndarray],
    n_v: np.ndarray,
    dtype: Any = np.float64,
) -> np.ndarray:
...
def _distance_metric(
    level_of_measurement: Union[str, Callable[..., np.ndarray]]
) -> Callable[..., np.ndarray]:
...
def _reliability_data_to_value_counts(
    reliability_data: np.ndarray, value_domain: np.ndarray
) -> np.ndarray:
    ...
    """
    return (
        reliability_data.T[..., np.newaxis] == value_domain[np.newaxis, np.newaxis, :]
    ).sum(
        axis=1
    )  # noqa
...
def alpha(
    ...
    if (reliability_data is None) == (value_counts is None):
        raise ValueError(
            "Either reliability_data or value_counts must be provided, but not both."
        )
    ...
    if value_counts is None:
        ...
        if value_domain is None:
            value_domain = np.unique(reliability_data[~pd.isna(reliability_data)])
        else:
            value_domain = np.asarray(value_domain)
            assert np.isin(
                reliability_data, np.append(value_domain, np.nan)
            ).all(), "The reliability data contains out-of-domain values."
        ...
        if value_domain is None:
            ...
            value_domain = np.asarray(value_domain)
            assert value_counts.shape[1] == len(
                value_domain
